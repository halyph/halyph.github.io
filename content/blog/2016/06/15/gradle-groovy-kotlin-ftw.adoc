...
You've porbaply heared about {my-link-gradle-kotlin}[_"Kotlin-based approach to writing Gradle build scripts"_]. A lot of noise was produced based on this announcement and Cédric Champeau tried to explain the real state of {my-link-cedric}[Groovy, Gradle and their future relationship].
...
** `buildSrc` - https://docs.gradle.org/current/userguide/organizing_build_logic.html#sec:build_sources[It's related to Gradle build process]
** `config` - nothing interesting: `condenarc` and `checkstyle` configs
** `design-docs` - markdown documentation
** `gradle` - Gradle scripts
** `subprojects` - Gradle sources. It's our goal to analyse this folder and understand the language statistics
